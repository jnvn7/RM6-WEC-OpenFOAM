#!/usr/bin/env python3
##--------------------------------------------------------------------------------#
# Script to plot the 6DoF Motion from  Motion.txt file generated by extractLog.sh
...
# Plotting outputs
fig, axs = plt.subplots(5,1,figsize=(8,8), tight_layout=True)
for case in sys.argv[1:]:
    time, surge, heave, pitch, roll, yaw = readData(os.path.join(case,"Motion.txt"))

    axs[0].plot(time, surge)
    axs[1].plot(time, heave)
    axs[2].plot(time, pitch)
    axs[3].plot(time, roll)
    axs[4].plot(time, yaw, label=case)

yLabels = ['Surge (m)','Heave (m)','Pitch (deg)', 'Roll (deg)', 'Yaw (deg)']
for i in range(5):
    axs[i].legend()
