...
```
should display the plot in Figure 1 below. 
...
```
should display the plot in Figure 2 below. 

&nbsp;
<p align="middle">
  <img src="https://github.com/jnvn7/RM6-WEC-OpenFoam/blob/main/images/plotWaves_compare_prepackaged_cases.png" height="300"/>
</p>
...
<p align="middle">
  <img src="https://github.com/jnvn7/RM6-WEC-OpenFoam/blob/main/images/plotWaves_compare_usergen_vs_prepack_case2.png" width="600"/>
</p>
...
Lastly, example files for performing a mesh resolution study are provided in the folder <tt>mesh_convergence_study</tt> for each of the cases. Within each case are subdirectories, containing instances of the case for varying mesh resolution sizes defined in units of CPH, meaning "cells per height", or cells per wave height. For example, a case named "4CPH" is set up so that the smallest grid cells in the domain (in areas of maximum refinement) have size (wave height)/4. 

Each folder contains pre-generated simulation probe data files in a <tt>prePackaged_postProcessing</tt> folder, which can be plotted with the <tt>plotWaves.py</tt> python script housed within the <tt>mesh_convergence_study</tt> folder. Example figures are provided in this directory showing the wave surface elevation with time for each refinement level and the percent change in wave height as the refinement level increases. 

For example, the plot for case 4 looks like Figure 4 below

<p align="middle">
  <img src="https://github.com/jnvn7/RM6-WEC-OpenFoam/blob/main/images/example_wave_flume_case4_mesh_resolution_study.png" width="600"/>
</p>
<p align='middle'> Figure 4 - Mesh resolution study conducted for example case 4. </p>